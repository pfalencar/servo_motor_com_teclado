...
char* password = "AD8"; //SENHA CORRETA PARA DESTRANCAR A FECHADURA
...
void loop(){
  ...
  if (key == '*' || key == '#') { //SE A TECLA PRESSIONADA POR IGUAL A CARACTERE "*" OU "#", fecha
...
if (locked){ //SE FECHADURA TRANCADA
    digitalWrite(ledVermelho, HIGH);
    digitalWrite(ledVerde, LOW);
    servo_Motor.write(0); //POSIÇÃO DO SERVO FICA EM 0º (FECHADO)
}
else{ //SENÃO, FAZ
    digitalWrite(ledVerde, HIGH);
    digitalWrite(ledVermelho, LOW);
    servo_Motor.write(90);// SERVO GIRA A 90º (ABERTO)
}
}